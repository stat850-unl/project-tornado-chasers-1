...
3. **Create Seasonal and Regional Aggregations:**
   - Aggregate tornado occurrences by month and region (state).

```{r}
# Aggregate tornado occurrences by month
tornado_monthly_counts <- aggregate(slat ~ mo, data = tornado_map_data, FUN = length)

# Aggregate tornado occurrences by state (assuming states are available in the dataset)
tornado_state_counts <- aggregate(slat ~ st, data = tornado_map_data, FUN = length)
```

4. **Create Interactive Maps:**
   - Use `tmap` to create interactive maps for monthly and regional variations.
```{r}
...
# Assuming your data frame is named 'tornado' and contains a column 'state_full'
...
# Merge tornado counts with U.S. state map
us_states <- merge(us_states1, tornado_counts, by.x = "ID", by.y = "state_full", all.x = TRUE)
...
leaflet(data = us_states) %>%
  ...
  addPolygons(
    fillColor = ~colorQuantile("Blues", us_states$tornado_count)(us_states$tornado_count),
    fillOpacity = 0.7,
    highlightOptions = highlightOptions(color = "white", weight = 2, bringToFront = TRUE),
    label = ~paste(state_abbr, "<br>", "Tornado Count:", tornado_count)
  ) %>%
  addLegend(
    position = "bottomright",
    pal = colorQuantile("Blues", us_states$tornado_count),
...
Note: In this example, the assumption is made that there is a dataset named `world` that contains geometries for world countries. You may need to replace it with the appropriate dataset for your region or country.

Adjust the code based on the specific structure of your data and your mapping requirements. Explore additional features of the `tmap` package for more customization options and interactivity.
