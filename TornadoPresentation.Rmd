...
1. Plot Tornado Frequency Over Time:
...
- The variables: latitude (`slat` and `elat`), longitude (`slon` and `elon`), and the month of occurrence (`mo`).

1.  Interactive plot for monthly tornado variation
...
2.  Interactive map using `leaflet` to visualize state-wise tornado count
...
- The variables: injuries, fatalities.
...
3. Interactive map using `leaflet` to visualize state-wise tornado Injuries and Fatalities.
...
3. Interactive map for state-wise total economic loss including Property loss and Crop loss.
...
- Findings from the analysis, accessible through the Shiny application, support informed decisions to mitigate the impact of tornadoes.
